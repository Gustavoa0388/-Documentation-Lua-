...
function CacaAoTesouro.TimerEvento()

	CacaAoTesouro.ClearByMonster()
	Timer.Interval(30, CacaAoTesouro.Init)
end

function CacaAoTesouro.Init()
	...
	then
		...
		if os.date("%X") == HORARIO_CACAAOTESOURO or os.date("%X") == HORARIO_CACAAOTESOURO2 or os.date("%X") == HORARIO_CACAAOTESOURO3
		then
			DataBase.SetValue3("EVENTO_AUTO", "status", 1) -- status evento ON
			CacaAoTesouro.CommandOpen_Auto()
		else
			return
		end
		...
		if EventoEscolhido == 'Caca ao Tesouro'
		then
			DataBase.SetValue3("EVENTO_AUTO", "status", 1) -- status evento ON
			CacaAoTesouro.CommandOpen_Auto()
		else
...
function CacaAoTesouro.CommandOpen_Auto()
	...
	elseif sorteioItem == 2
	then
		Item = "Creation"
		Section = 14
		Id = 22
	
	elseif sorteioItem == 3
	then
		Item = "Chaos"
		Section = 12
		Id = 15
	
	elseif sorteioItem == 4
	then
		Item = "Imp"
		Section = 13
		Id = 1
	
	elseif sorteioItem == 5
	then
		Item = "Guardian Angel"
		Section = 13
		Id = 0
		
	elseif sorteioItem == 6
	then
		Item = "Uniria"
		Section = 13
		Id = 2
	...
	started = true
	CacaAoTesouro.InitNPC()

	idtimer = Timer.Interval(1, CacaAoTesouro.Running)
	timer_finish = Timer.Interval(3 * 60, CacaAoTesouro.FinishEvent)
	
	SendMessageGlobal(string.format("[Sistema] Evento Caça ao Tesouro!"), 0)
...
function CacaAoTesouro.NpcTalk(Npc, Player)
	for i in pairs(TRADEWINS_NPC_CREATE) do
		if TRADEWINS_NPC_CREATE[i].NpcIndex == Npc
		then
			CacaAoTesouro.CacaTesouroNpc(Npc, Player)
			return 1
		end
	end
...
function CacaAoTesouro.CacaTesouroNpc(Npc, aIndex)
	...
	for i = 12, 75 do
		if InventoryIsItem(aIndex, i) == 1
			then
				if InventoryGetIndex(aIndex, i) == GET_ITEM(Section, Id)
				then
	...
	if items_uso < 1
	then
		ChatTargetSend(Npc, string.format("Você não tem o item que eu preciso! Volte aqui com: %s", Item), aIndex)
		return
	end
	...
	CacaAoTesouro.FinishEvent()
	
end



function CacaAoTesouro.InitNPC()
	...
	CacaAoTesouro.CreateNpc(249, Map, CoordX, CoordY, 3)
end

function CacaAoTesouro.CreateNpc(class, map, x, y, dir)
	index = AddMonster(map)
	
	if index == -1
	then
		LogAdd(string.format("[Caça ao Tesouro] Problema ao criar o Npc :%d", class))
		return
	end
...
function CacaAoTesouro.FinishEvent()

	SendMessageGlobal(string.format("[Sistema] Caça ao Tesouro Finalizado!"), 0)
	...
	then
		...
		for i, name in ipairs(Players) do 
			...
			DataBase.SetAddValue("Character", COLUNA_CACAAOTESOURO, PONTOS_EVENTINHOS, "Name", UserGetName(index))
	...
	for i in pairs(TRADEWINS_NPC_CREATE) do
		--Remover npc
		CacaAoTesouro.ClearByMonster(TRADEWINS_NPC_CREATE[i].NpcIndex)
	end
	...
	Timer.TimeOut(2, CacaAoTesouro.StatusOFFevent)
	
end


function CacaAoTesouro.ClearByMonster(index)
...
function CacaAoTesouro.Running()
	...
	SendMessageGlobal(string.format("[Caça ao Tesouro]  %s  ~> Item: %s", Move, Item), 0)
	
end

function CacaAoTesouro.CommandOpen(aIndex, Arguments)
	...
	then
		if gm == ''
		then
			SendMessage(string.format("[Sistema] Já existe um Caça ao Tesouro aberto pelo Sistema!"), aIndex, 1)
			return
		else
			SendMessage(string.format("[Sistema] Já existe um Caça ao Tesouro aberto pelo %s!", gm), aIndex, 1)
			return
		end
	end
...
function CacaAoTesouro.RemoveUserPlayers(Name)
...
function CacaAoTesouro.StatusOFFevent()
...
Commands.Register("/abrircaçatesouro", CacaAoTesouro.CommandOpen)
GameServerFunctions.NpcTalk(CacaAoTesouro.NpcTalk)

CacaAoTesouro.TimerEvento()

return CacaAoTesouro